...
# One aligned menu row: [key] name  description
function Write-MenuRow ($key, $name, $desc, $col) {
    Write-Host ("      {0,4}  " -f "[$key]") -NoNewline -ForegroundColor $col
    Write-Host ("{0,-16}" -f $name) -NoNewline -ForegroundColor White
    Write-Host $desc -ForegroundColor DarkGray
}

function Write-Group ($t) { Write-Host "`n   -- $t " -NoNewline -ForegroundColor DarkCyan; Write-Host ("-" * (48 - $t.Length)) -ForegroundColor DarkCyan }
...
do {
    ...
    Write-Group "SETUP & SHARING"
    Write-MenuRow "1"  "Cleanup"       "Remove old mappings, credentials, cache" Yellow
    Write-MenuRow "2"  "Setup"         "Create share + user + map drives"        Green
    Write-MenuRow "9"  "Map Drives"    "Only re-map drives (2nd pass)"           Green

    Write-Group "NETWORK / IP"
    Write-MenuRow "3"  "Harden"        "LAN priority + router-proof settings"    Cyan
    Write-MenuRow "8"  "Set IP by Name" "Name-based IP + full per-PC setup"      White
    Write-MenuRow "5"  "Auto IP"       "Auto series, keep current last number"   White
    Write-MenuRow "4"  "IP Series"     "Enter series + last number manually"     Magenta
    Write-MenuRow "6"  "Hotspot Net"   "Router dead? internet via hotspot"       Blue

    Write-Group "TOOLS"
    Write-MenuRow "7"  "Health Check"  "Diagnose + auto-fix everything"          Red
    Write-MenuRow "10" "Accounts"      "Create / delete local accounts"          DarkYellow
