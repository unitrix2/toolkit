...
function Repair-Firewall {
    ...
    # built-in sharing/discovery ON for PRIVATE/DOMAIN only -- keep PUBLIC off (secure)
    foreach ($grp in "File and Printer Sharing","Network Discovery") {
        Get-NetFirewallRule -DisplayGroup $grp -ErrorAction SilentlyContinue |
            Where-Object { $_.Profile -notmatch 'Public' } | Enable-NetFirewallRule -ErrorAction SilentlyContinue
        Get-NetFirewallRule -DisplayGroup $grp -ErrorAction SilentlyContinue |
            Where-Object { $_.Profile -match 'Public' }    | Disable-NetFirewallRule -ErrorAction SilentlyContinue
    }
...
function Optimize-Network {
    ...
    Repair-Firewall
    Write-Ok "Firewall: sharing ON for Private (Public stays OFF), LAN-only allow-rules (in+out), ping ON."
