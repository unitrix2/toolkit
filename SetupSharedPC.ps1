...
function Connect-Drive ($letter, $ip, $label) {
    ...
    # 2) Quick reachability check (avoids 30-60 sec net.exe hang)
    $reachable = Test-Connection -ComputerName $ip -Count 1 -Quiet -ErrorAction SilentlyContinue
    $portOk    = $false
    if ($reachable) {
        $portOk = (Test-NetConnection -ComputerName $ip -Port 445 -InformationLevel Quiet -WarningAction SilentlyContinue)
    }
