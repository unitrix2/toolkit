...
function Invoke-Cleanup {
    ...
    # active + dead dono mapped drives hatao
    ...
    foreach ($code in 69..90) {   # E: se Z: tak force delete (local disk ignore ho jayega)
        & net.exe use ("{0}:" -f [char]$code) /delete /y *>$null
    }
    Write-Ok "Saari mapped drives delete (dead/atki hui bhi)."
    ...
    Write-Info "Koi drive phir bhi dikhe? Wo aapki login-session me atki hai (registry saaf ho chuka)."
    Write-Info "Ek baar LOGOFF/RESTART karo -- pakka hat jayegi, wapas nahi aayegi."
}
