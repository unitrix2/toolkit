...
while ($running) {
    ...
    try { $menuItems = Load-Menu }
        ...
        Write-Host ""
        Write-Host "  [ERR] menu.txt load nahi hua: $($_.Exception.Message)" -ForegroundColor Red
        Write-Host "  GitHub repo aur internet check karo." -ForegroundColor Yellow
        Read-Host "`n  Enter dabao retry karne ke liye"
        continue
    }
    ...
    if (-not [int]::TryParse($choice, [ref]$intChoice)) {
        Write-Host "  Galat input." -ForegroundColor Red; Start-Sleep -Seconds 1; continue
    }

    $sel = $menuItems | Where-Object { $_.Num -eq $intChoice } | Select-Object -First 1
    if ($null -eq $sel) {
        Write-Host "  Galat option." -ForegroundColor Red; Start-Sleep -Seconds 1; continue
    }
    ...
    try {
        ...
        # External URL (http/https) ho to direct use karo, warna apni repo se lo
        ...
        Write-Host "  [ERR] Feature load nahi hua: $($_.Exception.Message)" -ForegroundColor Red
